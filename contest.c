...
void kuozhang(struct haha*psp)
{
	...
	if (ftell(pfread) != 0)
	{
		fseek(pfread, 0, SEEK_SET);
		 while(fscanf(pfread, "%s %s %s %s %d",
			psp->data[psp->size].name,
			psp->data[psp->size].sex,
			psp->data[psp->size].number,
			psp->data[psp->size].adress,
			&(psp->data[psp->size].age))==5)
		{
			psp->size++;
			kuorong(psp);
		}
...
void storage(struct haha* psp)
{
	...
	fscanf(pfread, "%-20s%-10s%-20s%-20s%-10d",
		psp->data[psp->size-1].name,
		psp->data[psp->size-1].sex,
		psp->data[psp->size-1].number,
		psp->data[psp->size-1].adress,
		&(psp->data[psp->size-1].age));
